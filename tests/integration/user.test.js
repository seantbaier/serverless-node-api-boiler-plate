...
describe('User routes', () => {
  ...
  describe(`GET ${baseUrl}/users/:userId`, () => {
    test('should return 200 and the user object if data is ok', async () => {
      await insertUsers([userOne])
      const res = await request(app)
        .get(`${baseUrl}/users/${userOne._id}`)
        .set('Authorization', `Bearer ${userOneAccessToken}`)
        .send()
        .expect(httpStatus.OK)
      expect(res.body).not.toHaveProperty('password')
  ...
  describe(`DELETE ${baseUrl}/users/:userId`, () => {
    test('should return 204 if data is ok', async () => {
      await insertUsers([userOne])
      await request(app)
        .delete(`${baseUrl}/users/${userOne._id}`)
        .set('Authorization', `Bearer ${userOneAccessToken}`)
        .send()
    ...
    test('should return 403 error if user is trying to delete another user', async () => {
      await insertUsers([userOne, userTwo])
      await request(app)
        .delete(`${baseUrl}/users/${userTwo._id}`)
        .set('Authorization', `Bearer ${userOneAccessToken}`)
        .send()
        .expect(httpStatus.FORBIDDEN)
    })
    ...
    test('should return 400 error if userId is not a valid mongo id', async () => {
      await insertUsers([admin])
      await request(app)
        .delete(`${baseUrl}/users/invalidId`)
        .set('Authorization', `Bearer ${adminAccessToken}`)
        .send()
        .expect(httpStatus.BAD_REQUEST)
    })
  ...
  describe(`PATCH ${baseUrl}/users/:userId`, () => {
    test('should return 200 and successfully update user if data is ok', async () => {
      ...
      const res = await request(app)
        .patch(`${baseUrl}/users/${userOne._id}`)
        .set('Authorization', `Bearer ${userOneAccessToken}`)
        .send(updateBody)
        .expect(httpStatus.OK)
      expect(res.body).not.toHaveProperty('password')
    ...
    test('should return 403 if user is updating another user', async () => {
      ...
      const updateBody = { name: faker.name.findName() }
      await request(app)
        .patch(`${baseUrl}/users/${userTwo._id}`)
        .set('Authorization', `Bearer ${userOneAccessToken}`)
        .send(updateBody)
        .expect(httpStatus.FORBIDDEN)
    })
